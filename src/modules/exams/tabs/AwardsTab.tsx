    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Détection automatique des meilleurs élèves, progressions et majors par discipline</p>
        </div>
        ...
        <div className="flex items-center space-x-3 w-full sm:w-auto justify-end">
          <button
            ...
            className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-amber-300 rounded-xl text-xs font-semibold flex items-center space-x-2 transition-colors border border-amber-500/20 disabled:opacity-50"
          >
          ...
          <button
            ...
            className="px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-brand-500/20 flex items-center space-x-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {awards.map((award) => (
          <div key={award.id} className="bg-slate-900 border border-slate-800 hover:border-amber-500/40 rounded-2xl p-5 shadow-xl transition-all space-y-4 relative overflow-hidden flex flex-col justify-between">
            <div className="space-y-3">
              ...
              <div>
                <h3 className="text-base font-extrabold text-white">{award.title}</h3>
                <p className="text-xs text-slate-400 mt-1 line-clamp-2">{award.description}</p>
              </div>

              <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-1">
                <div className="font-bold text-slate-100 text-sm">{award.student_name}</div>
                <div className="text-xs text-slate-400 flex items-center justify-between">
                  <span>{award.class_name} • {award.registration_number}</span>
                  {award.average && <span className="font-extrabold text-amber-400">{award.average.toFixed(2)} / 20</span>}
                </div>
              </div>
            </div>

            <div className="pt-3 border-t border-slate-800 flex items-center justify-between text-xs">
              <span className="text-slate-500 italic">Décerné le {new Date(award.awarded_at || Date.now()).toLocaleDateString('fr-FR')}</span>
              <button
                onClick={() => onGenerateCert(award)}
                className="px-3 py-1.5 bg-brand-500/20 hover:bg-brand-500 text-brand-300 hover:text-white font-bold rounded-lg transition-colors flex items-center space-x-1"
              >
